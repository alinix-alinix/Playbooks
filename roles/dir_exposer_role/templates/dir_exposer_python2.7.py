...
class AuthHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def _send_directory_listing(self, directory_path, requested_path):
        ...
        for filename in files:
            ...
            if os.path.isdir(file_path):
                ...
                self.wfile.write('<tr><td><a href="/{0}/">{0}</a></td><td>-</td><td>{1}</td></tr>'.format(
                    requested_path + "/" + filename, file_mtime_str).encode())
            else:
                # Для файлов создаем ссылку для скачивания
                self.wfile.write('<tr><td><a href="/{0}">{0}</a></td><td>{1} bytes</td><td>{2}</td></tr>'.format(
                    requested_path + "/" + filename, file_size, file_mtime_str).encode())
    ...
    def _is_authenticated(self, auth):
        ...
        try:
            auth_string = base64.decodestring(auth_string.strip())  # decode для Python 2.7
